...
export default function RoomCard({ room, onViewMore, onEdit, onDelete, onViewQuestions, onUploadEvaluation }) {
  ...
  const handleViewRoomId = () => {
    Alert.alert("Código de la Sala", `Nombre: ${room.name}\n\nCódigo: ${room.short_code || room.id}`, [
      {
        text: "Copiar Código",
        onPress: () => {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.roomInfo}>
          ...
          <View style={styles.roomNameContainer}>
            <View style={styles.roomHeader}>
              ...
              {room.max_score ? (
                <View style={[styles.maxScoreBadge, { backgroundColor: room.color || "#4361EE" }]}>
                  <Feather name="star" size={12} color="#fff" />
                  <Text style={styles.maxScoreText}>{room.max_score}</Text>
                </View>
              ) : null}
            </View>
            {room.course ? <Text style={styles.roomCourse}>{room.course}</Text> : null}
            {room.description ? (
          ...
          <TouchableOpacity ref={moreButtonRef} style={styles.moreButton} onPress={handleMenuPress}>
      ...
      <Modal visible={showMenu} transparent={true} animationType="fade" onRequestClose={() => setShowMenu(false)}>
        <TouchableOpacity style={styles.popoverOverlay} activeOpacity={1} onPress={() => setShowMenu(false)}>
          <View
            style={[
              styles.popoverMenu,
              {
