...
from utils.config import ROOM_TABLE, ROLES_PERMITTED_GET_ROOM, ROLES_PERMITED_CREATE_ROOM, ROOM_GSI_INDEX_SHORTCODE
...
def lambda_handler(event, context):
    try:
        ...
        if not pathParameter or 'code' not in pathParameter:
            logger.error("Falta el parámetro 'code' en la solicitud.")
            return Response(status_code=400, body={"error": "Falta el parámetro 'code' en la solicitud."}).to_dict()

        short_code = pathParameter.get('code')

        # Buscar usando el índice GSI por short_code
        ...
        if 'Items' not in response or len(response['Items']) == 0:
            logger.error(f"Room no encontrado con shortCode: {short_code}")
            return Response(status_code=404, body={'error': 'Room no encontrado.'}).to_dict()

        room_data = serialize_dynamo_to_dict(response['Items'][0])

        if role not in ROLES_PERMITTED_GET_ROOM:
            logger.error(f"Acceso no autorizado para el usuario {user_id} a la room con shortCode: {short_code}")
        ...
        if role in ROLES_PERMITED_CREATE_ROOM and room_data["user_id"] != user_id:
            logger.error(f"El usuario {user_id} no es el creador de la room con shortCode: {short_code}")
            return Response(status_code=403, body={"error": "Acceso no autorizado al room."}).to_dict()
